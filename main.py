...
def train():
    ...
    for _ in range(100):  # Play 100 random steps
        ...
        if not done:
            s_tensor = torch.tensor(state, dtype=torch.uint8, device="cpu")
    ...
    while steps_done < Config.TOTAL_FRAMES:
        ...
        while True:
            epsilon = Config.EPS_END + (Config.EPS_START - Config.EPS_END) * max(
                0, (Config.EPS_DECAY_FRAMES - steps_done) / Config.EPS_DECAY_FRAMES
            )
            ...
            optimize_model(policy_net, target_net, memory, optimizer, Config.DEVICE)
